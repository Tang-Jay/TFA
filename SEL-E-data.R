#==================================================#
#  SEL-E-data：指数 DGP 功效模拟（SEL.tex §4, test-1）#
#  X_i ~ Exp(1/mu)，mu in {0.05, 0.06, ..., 0.45}   #
#==================================================#
rm(list = ls())
library(emplik)
...
test_config <- function(test, mu0 = 0.25, mu1 = NULL, mu2 = NULL, mu_grid = NULL) {
  ...
  cfg <- switch(
    test,
    test1 = list(
      test = test, tex_ref = "§4, test-1, Exp(1/mu)",
...
make_dgp_rng <- function(dgp) {
  switch(
    dgp,
    exp = function(n, mu) {
      if (mu <= 0) stop("Exp DGP 要求 mu > 0，当前 mu = ", mu)
      rexp(n, rate = 1 / mu)
    },
    stop("未知 dgp: ", dgp)
  )
...
simulate_reject <- function(test, n, mu, mu0, mu1, mu2, an, a, cut_T1, cut_T2, rng = NULL) {
  test <- normalize_test(test)
  if (is.null(rng)) rng <- make_dgp_rng("exp")
  n1 <- floor(n / 2)
  X <- rng(n, mu)
  X1 <- X[seq_len(n1)]
  X2 <- X[(n1 + 1):n]
...
test <- "test1"
run_all_tests <- FALSE
mu_grid_test1 <- list(mu_l = 0.05, mu_u = 0.45, mu_step = 0.01)
...
for (test_cur in tests) {
  ...
  if (test_cur != "test1" && dgp == "exp") {
    warning("Exp DGP 仅适用于 mu>0，跳过 ", test_cur)
    next
  }
  run_power_sim(
