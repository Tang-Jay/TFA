#==================================================#
#  SEL-C-data：功效模拟并保存 .RData              #
#  对应 02 任务/SEL/手稿/SEL.tex：test-1（§3）、test-2/3（§3.3）#
#  参数 test：test1 / test2 / test3（greater / less / or）#
#  输出：SEL-C-{test}-nsim-...-{n}.RData / .png              #
#==================================================#
rm(list = ls())
library(emplik)

#=================================================#
#  test 归一化与检验设定（SEL.tex）
#=================================================#
normalize_test <- function(test) {
  ...
  switch(
    ...
    "or" = "test3",
    "双侧" = "test3",
    stop("test 须为 test1 / test2 / test3 之一（对应 greater / less / or），当前为: ", test)
  )
}

test_config <- function(test, mu0 = 0.25, mu1 = NULL, mu2 = NULL) {
...
resolve_an <- function(n, an_method, a2, a) {
  switch(
    ...
    "logn/n" = log(n) / n,
    "loglogn" = log(log(n)),
    stop("an_method 必须是 'qnorm'、'theory'、'logn'、'logn/2'、'logn/n' 或 'loglogn/n'")
  )
}

# 单次模拟：返回是否拒绝（逻辑标量）
default_rng <- function(test) {
  test <- normalize_test(test)
  if (test == "test1") {
    function(n, mu) rchisq(n, df = mu)
  } else {
    function(n, mu) rnorm(n, mean = mu, sd = 1)
  }
}

simulate_reject <- function(test, n, mu, mu0, mu1, mu2, an, a, cut_T1, cut_T2,
                            rng = NULL) {
  test <- normalize_test(test)
  if (is.null(rng)) rng <- default_rng(test)
  n1 <- floor(n / 2)
  X <- rng(n, mu)
  X1 <- X[seq_len(n1)]
  X2 <- X[(n1 + 1):n]
...
run_power_sim <- function(test, ns, nsim, a, a2, mu0, mu1, mu2,
                          ...
                          rng = NULL) {
  test <- normalize_test(test)
  cfg <- test_config(test, mu0 = mu0, mu1 = mu1, mu2 = mu2)
  ...
  for (n in ns) {
    ...
    if (save_data) {
      data_path <- file.path(
        power_dir,
        paste0(stem, "-nsim-", nsim, "-a2-", a2_file, "-n-", n, ".RData")
      )
      mu_l <- cfg$mu_l
      mu_u <- cfg$mu_u
      save(
    ...
    if (save_plot) {
      file_path <- file.path(
        power_dir,
        paste0(stem, "-nsim-", nsim, "-a2-", a2_file, "-n-", n, ".png")
      )
...
show_plot <- TRUE   # 计算后在当前图形设备出图（RStudio Plots 窗格，不弹独立窗口、不存 png）
save_plot <- FALSE  # TRUE 时另存 png
power_dir <- "."

# test：test1（greater）、test2（less）、test3（or）；run_all_tests=TRUE 时三种都跑
test <- "test1"
run_all_tests <- TRUE
