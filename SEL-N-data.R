#==================================================#
#  SEL-N-data：正态 DGP 功效模拟（SEL.tex §4, test-1）#
#  输出：SEL-N-{test}-nsim-...-{n}.RData / .png              #
#  X_i ~ N(mu, 1)，mu in {0, 0.01, ..., 0.5}       #
#==================================================#
rm(list = ls())
library(emplik)
...
test_config <- function(test, mu0 = 0.25, mu1 = NULL, mu2 = NULL, mu_grid = NULL) {
  ...
  cfg <- switch(
    test,
    test1 = list(
      test = test, tex_ref = "§4, test-1, N(mu,1)",
...
simulate_reject <- function(test, n, mu, mu0, mu1, mu2, an, a, cut_T1, cut_T2, rng = NULL) {
  test <- normalize_test(test)
  if (is.null(rng)) rng <- make_dgp_rng("normal")
  n1 <- floor(n / 2)
  X <- rng(n, mu)
  X1 <- X[seq_len(n1)]
  X2 <- X[(n1 + 1):n]
